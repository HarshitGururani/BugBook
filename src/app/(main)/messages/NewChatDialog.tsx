...
export default function NewChatDialog({
  ...
  const seacrhInputDebounced = useDebounce(searchInput);
      ...
      client.queryUsers(
        {
          ...
          ...(seacrhInputDebounced
            ? {
                $or: [
                  { name: { $autocomplete: seacrhInputDebounced } },
                  { username: { $autocomplete: seacrhInputDebounced } },
                ],
              }
            : {}),
        },
  ...
  const mutation = useMutation({
    ...
    onError(error) {
      console.log("Error  strating chat. Please try again.");
      toast({
        variant: "destructive",
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="bg-card p-0">
        <DialogHeader className="px-6 pt-6">New Chat</DialogHeader>
        <div>
          <div className="group relative">
            <SearchIcon className="top1/2 absolute left-5 size-5 -translate-y-1/2 transform text-muted-foreground group-focus-within:text-primary" />
